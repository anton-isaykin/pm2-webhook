    ...
    for (var name in webhooks) {
        ...
        var checkOrigin = new Promise(function(resolve, reject){
            ...
            try {
                ...
                origin = _.result(origin, 'ref', '').toString().split('/').slice(-1)[0];
            } catch (e){
        ...
        checkOrigin
            .then(function (isCorrectOrigin) {
                if (isCorrectOrigin) {
                    ...
                    console.log('Webhook from invalid branch (Application:', name, ')');
                }
            })
            .catch(function () {
        ...
        function (callback) {
            ...
            pm2.describe(name, function (err, apps) {
                ...
                exec(current_app.post_hook, {cwd: cwd}, function (err, stdout, stderr) {
                    ...
                    console.log('Post-hook command has been successfully executed for app', name);
                    return callback(null);
                })
            })
        }
