    ...
    pm2.connect(function () {
        ...
        pm2.launchBus(function (err, bus) {
            bus.on('process:event', function (proc) {
                console.log(proc.process.name, proc.event, proc.manually);

                    //console.log(eventName,'::',proc.process?proc.process.name:'-','',proc.process?proc.process.status:proc.process);

          /*      if (args && args.manually === false) {
                    console.log('pm2-webhook::event skipped', args.event, args.process?args.process.name:"");
                    return;
                }*/

                // console.info('pm2-webhook::', 'event happened', args.event, args.process?args.process.name:"" );
                //
